...
─────────────────────────────────────────────────────────────────
Discharge Summary Generation Agent — Clinical Insight Reporter Tool

Reads:  normalized data, completeness result, ehr_validation result
Uses:   configs/rules.yaml  (risk weights + thresholds + recommendations)
Returns:
    {
        ...
        "risk_score"      : int,
        "recommendation"  : str,
        "flags"           : [{"rule": str, "weight": int, "detail": str}],
        "hard_blocked"    : bool,          # True if any hard guardrail hit
        "hard_block_reasons": [...],
        "summary"         : str,           # patient-friendly discharge summary
        "audit_trail"     : [...],         # timestamped agent decisions
        "rules_version"   : str,           # SHA-256 of rules.yaml
        "generated_at"    : str,
    }
"""
...
# ── Load rules.yaml once at import time ───────────────────────
...
# ═══════════════════════════════════════════════════════════════
#  Public entry point  (called by reporting_node in the graph)
# ═══════════════════════════════════════════════════════════════
...
def generate_report(
    ...
    """
    Args:
        normalized      : output of translation_normalization_agent
                          (the merged, English, normalised patient payload)
        completeness    : output of completeness_agent
        ehr_validation  : output of ehr_validation_agent

    Returns a report dict — see module docstring.
    """
    audit_trail = []
    ...
    def _flag(rule: str, detail: str, override_weight: int = None):
        """Record a risk flag, accumulate score, check hard guardrail."""
    ...
    # ── 3. Allergy contradiction hard check ───────────────────
    ...
    # ── 4. High-risk med missing in EHR ──────────────────────
    ...
    # ── 5. High-risk med — no counseling note ─────────────────
    ...
    # ── 9. Bill unpaid + discharge approved ───────────────────
...
# ═══════════════════════════════════════════════════════════════
#  Patient-friendly summary builder
# ═══════════════════════════════════════════════════════════════

def _build_summary(normalized: dict, risk_level: str, recommendation: str) -> str:
    """
    Builds a plain-English discharge summary.
    Replaces medical abbreviations using the normalization map in rules.yaml.
    """
...
def _get_med_names(normalized: dict) -> list[str]:
    """Extract medicine names from normalized medication list."""
    meds = normalized.get("medications", [])
